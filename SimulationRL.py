...
def simProgress(simTimelimit, env, earth1):
    timeSteps = 100
    ...
    while True:
        env.process(earth1.save_plot_at_intervals(env,img_count, interval=1))
        img_count += 1
...
class Earth:
    ...
    def save_plot_at_intervals(self, env,img_count, interval=1 ):
        while True:
...
def RunSimulation(GTs, inputPath, outputPath, populationData, radioKM):
    ...
    for GTnumber in GTs:
        ...
        progress = env.process(simProgress(simulationTimelimit, env, earth1))
        startTime = time.time()
