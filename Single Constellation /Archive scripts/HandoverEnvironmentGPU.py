...
class LEOEnv(gym.Env):
    ...
    def __init__(self, input_csv=None, movementTime=20):
        ...
        if input_csv is None:
            ...
            input_csv = os.path.join(base_dir, "Inputs", "input.csv")
    ...
    def step(self, action):
        ...
        self.earth.step_aircraft(self.deltaT)
...
# def predict_valid_action(model, obs, mask): #for CPU
...
def predict_valid_action(model, obs, mask):
    """Manually ensure only valid actions are predicted"""
    # Move obs_tensor to the same device as the model's policy
    device = next(model.policy.parameters()).device
    obs_tensor = torch.tensor(obs, dtype=torch.float32, device=device).reshape(1, -1)
    ...
    # Get the action logits
    distribution = model.policy.get_distribution(obs_tensor)
    logits = distribution.distribution.logits.detach().cpu().numpy().flatten()
