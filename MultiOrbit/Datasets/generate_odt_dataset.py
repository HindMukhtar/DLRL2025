...
def _collect_trajectories(env, predict_fn, model, episodes, quiet=True, scenario=None, source=None, seed_base=42):
    trajectories = []
    for ep in range(episodes):
        ...
        with cm:
            ...
            while not (done or truncated):
                ...
                if not np.any(mask):
                    # Progress simulation even when no valid steering action exists yet.
                    action = 0
                elif model is None:
...
def _serialize_action(action):
    """
    Convert scalar/vector action into a single discrete index for offline dataset compatibility.
    """
...
def _mask_to_1d(mask):
    mask_arr = np.asarray(mask)
    if mask_arr.ndim <= 1:
        return mask_arr.astype(bool)
    return np.any(mask_arr.astype(bool), axis=0)
...
def _oracle_action(env):
    base_env = env.env
    mask = _mask_to_1d(base_env._get_action_mask())
    ...
    for i, candidate in enumerate(candidates):
        if i >= len(mask) or not mask[i] or candidate is None:
            continue
    ...
    for seed in dataset_seeds:
        ...
        for scenario in scenarios:
            ...
            if episode_budget.get("ppo", 0) > 0:
                ...
                ppo_env = PPOEnv(constellation_name, route, scenario=scenario, seed=seed)
            ...
            if episode_budget.get("dqn", 0) > 0:
                ...
                dqn_env = DQNEnv(constellation_name, route, scenario=scenario, seed=seed)
            ...
            if episode_budget.get("oracle", 0) > 0:
                ...
                oracle_env = PPOEnv(constellation_name, route, scenario=scenario, seed=seed)
            ...
            if episode_budget.get("oracle_latency", 0) > 0:
                ...
                oracle_latency_env = PPOEnv(constellation_name, route, scenario=scenario, seed=seed)
            ...
            if episode_budget.get("oracle_drop", 0) > 0:
                ...
                oracle_drop_env = PPOEnv(constellation_name, route, scenario=scenario, seed=seed)
