...
from Training.ODT import ExperienceBuffer, OnlineDecisionTransformer
...
def main():
    ...
    datasets_dir = os.path.join(base_dir, DATASETS_DIRNAME)
    models_dir = os.path.join(base_dir, MODELS_DIRNAME)
    os.makedirs(dataset_dir, exist_ok=True)
    os.makedirs(datasets_dir, exist_ok=True)
    os.makedirs(models_dir, exist_ok=True)
    dataset_candidates = [
        os.path.join(datasets_dir, "odt_offline_dataset_merged.pkl"),
        os.path.join(datasets_dir, "odt_offline_dataset_aug.pkl"),
        os.path.join(dataset_dir, "odt_offline_dataset_merged.pkl"),
        os.path.join(dataset_dir, "odt_offline_dataset_aug.pkl"),
        os.path.join(base_dir, "odt_offline_dataset_merged.pkl"),
        os.path.join(base_dir, "odt_offline_dataset_aug.pkl"),
    ]
    existing_candidates = [p for p in dataset_candidates if os.path.exists(p)]
    if not existing_candidates:
        raise FileNotFoundError(
            "Could not find offline dataset. Checked:\n" + "\n".join(dataset_candidates)
        )
    dataset_path = None
    trajectories = None
    candidate_summaries = []
    for p in existing_candidates:
        try:
            ...
            candidate_summaries.append(f"{p} (len={n})")
            if n and n > 0 and dataset_path is None:
                dataset_path = p
                trajectories = d
        except Exception as e:
            candidate_summaries.append(f"{p} (unreadable: {e})")

    if dataset_path is None:
        raise RuntimeError(
            "All discovered offline datasets are empty or unreadable:\n" + "\n".join(candidate_summaries)
        )
