...
from Environment.LEOEnvironmentRL import initialize, load_route_from_csv
from Training.HandoverEnvironment import mask_fn
...
class LEOEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Advance simulation
        qoe_list = self.earth.step_aircraft(actions=action_indices, folder="ODT")
        self.last_qoe = qoe_list[0] if qoe_list else {}
...
class LEOEnvDecisionTransformer(gym.Env):
    ...
    def __init__(self, constellation_name, route, model_path, scenario=None):
        super().__init__()
        
        # Use your existing environment setups
        self.base_env = LEOEnv(constellation_name, route, scenario=scenario)
...
def main():
    ...
    if not os.path.exists(input_path):
        input_path = os.path.join(project_dir, "input.csv")
    inputParams = pd.read_csv(input_path)
    constellation_name = inputParams['Constellation'][0]
    route_candidates = [
        os.path.join(project_dir, "routes", "route_5s_interpolated.csv"),
        os.path.join(project_dir, "route_5s_interpolated.csv"),
        os.path.join(os.path.dirname(project_dir), "Single Constellation ", "routes", "route_5s_interpolated.csv"),
        os.path.join(os.path.dirname(project_dir), "Single Constellation", "routes", "route_5s_interpolated.csv"),
        os.path.join(os.path.dirname(project_dir), "Single Constellation ", "routes", "route.csv"),
    ]
    ...
    scenario = None
    base_env = LEOEnvDecisionTransformer(constellation_name, route, model_path=None, scenario=scenario)
