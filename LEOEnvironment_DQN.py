...
class AircraftSNR: 
    ...
    def calculate_allocation(self, deltaT):
...
class AircraftDQN: 
    ...
    def __init__(self, env, aircraft_id, start_lat, start_lon, height, speed_kmph, direction_deg, update_interval=1):
        ...
        self.N_CANDIDATE_BEAMS = 5
        self.N_FEATURES_PER_BEAM = 5
    ...
    def observe_state_and_candidates(self, earth, threshold_km=1500):
        ...
        if not beam_info:
            return [], np.zeros((1, self.agent.state_size))

        beam_info.sort(key=lambda x: x['snr'], reverse=True)
        candidates = beam_info[:self.N_CANDIDATE_BEAMS]
        ...
        padding = [0] * (self.agent.state_size - len(state_vector))
        state_vector.extend(padding)

        return [info['beam'] for info in candidates], np.array(state_vector).reshape(1, -1)
    ...
    def step(self, earth, deltaT):
        ...
        if action < len(candidate_beams):
            ...
            # Ensure connected_satellite is not None before calculating SNR and Latency
...
class Earth:
    def __init__(self, env, img_path, constellation, aircrafts, inputParams, deltaT, window=None):
        ...
        self.step_aircraft_process = env.process(self.step_aircraft(env, deltaT)) # Renamed to avoid conflict with method name
        ...
        while True:
            ...
            for ac in self.aircrafts:
                ...
                if isinstance(ac, AircraftSNR):
                    ...
                    ratio, allocated, demand, beam_capacity_MB = ac.calculate_allocation(deltaT)
                    ...
                    if ac.connected_beam:
                        print(f"  Connected to {ac.connected_beam.id}. SNR: {ac.current_snr:.2f} dB, Latency: {ac.current_latency*1e3:.2f} ms")
                        print(f"  total reward is {ac.self.total_reward}")
                    else:
                        print("  No connection.")
