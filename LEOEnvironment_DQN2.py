...
class AircraftSNR: 
    ...
    def allocation_ratio(self, deltaT):
        ...
        self.demand = self.get_demand(deltaT)
...
class AircraftDQN: 
    ...
    def observe_state_and_candidates(self, earth):
        """
        Observes the environment to create a state vector for the DQN agent.
        Selects the N_CANDIDATE_SATELLITES nearest satellites and their beams.
        """
        ...
        # Sort satellites by distance and select the N_CANDIDATE_SATELLITES nearest ones
        ...
        for sat in nearest_satellites:
            for beam in sat.beams:
                aircraft_point = Point(self.longitude, self.latitude)
                if beam.get_footprint_eclipse().contains(aircraft_point):
                    dist_3d = self._calculate_3d_distance(sat) # Recalculate for consistency, though 'sat' already passed it
                    snr = self.calculate_snr(beam, dist_3d / 1000)
                    latency = self._calculate_latency(sat)
                    beam_info.append({
                        "sat": sat, "beam": beam, "snr": snr, "latency": latency,
                        "rel_lat": beam.center_lat - self.latitude,
                        "rel_lon": beam.center_lon - self.longitude
                    })
        
        # Sort collected beams by SNR (optional, but good for consistent state ordering)
        beam_info.sort(key=lambda x: x['snr'], reverse=True)

        # Pad or truncate the list of candidate beams to N_CANDIDATE_BEAMS
        candidates_for_state = []
        for i in range(self.N_CANDIDATE_BEAMS):
            if i < len(beam_info):
                candidates_for_state.append(beam_info[i])
            else:
                # Create a "dummy" beam for padding, with placeholder values
                # This ensures the state vector always has a consistent size
                dummy_beam = Beam(center_lat=0, center_lon=0, width_deg=0.1, height_deg=0.1, 
                                  load=0, snr=-np.inf, id=f"dummy_beam_{i}")
                dummy_info = {
                    "sat": None, "beam": dummy_beam, "snr": -np.inf, "latency": 1000, # High latency for bad connection
                    "rel_lat": 0, "rel_lon": 0
                }
                candidates_for_state.append(dummy_info)

        state_vector = []
        for info in candidates_for_state:
            state_vector.extend([
                info['rel_lat'], info['rel_lon'], info['snr'], 
                info['beam'].capacity, info['latency'] * 1e3
            ])
        
        return [info['beam'] for info in candidates_for_state], np.array(state_vector).reshape(1, -1)
